...
class UserBTConfig : AppCompatActivity() {
    ...
    private val scanResultAdapter: ScanResultAdapter by lazy {
        ScanResultAdapter(scanResults) { result ->
            ...
            with(result.device) {
                ...
                ConnectionManager.connect(this, this@UserBTConfig)
            }
        }
    }
    ...
    private val scanCallback = object : ScanCallback() {
        override fun onScanResult(callbackType: Int, result: ScanResult) {
            ...
            if (indexQuery != -1) { // A scan result already exists with the same address
                ...
                with(result.device) {
                    if (ActivityCompat.checkSelfPermission(this@UserBTConfig, Manifest.permission.BLUETOOTH_CONNECT) != PackageManager.PERMISSION_GRANTED) {
                        // TODO: Consider calling
                        //    ActivityCompat#requestPermissions
                        // here to request the missing permissions, and then overriding
                        //   public void onRequestPermissionsResult(int requestCode, String[] permissions,
    ...
    private val connectionEventListener by lazy {
        ConnectionEventListener().apply {
            onConnectionSetupComplete = { gatt ->
                Intent(this@UserBTConfig, BleOperationsActivity::class.java).also {
